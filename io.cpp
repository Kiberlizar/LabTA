...
void readData(Audience *aud, int count) {
  ...
  for (int i = 0; i < count; i++) {
    cout << "Введіть дані для аудиторії #" << i + 1
         << " (номер, кількість місць, наявність дошки (1 або 0), кількість "
            "комп'ютерів): ";
...
/**
 * @brief Виводить результати: відсортований список аудиторій та статистику за
 * місткістю.
 * @param aud Вказівник на масив структур Audience.
...
void printResults(Audience *aud, int count, int *capacities,
  ...
  for (int i = 0; i < count; i++) {
    cout << "Аудиторія №" << aud[i].number << ", місць: " << aud[i].seats
         ...
         << (aud[i].board
                 ? "є"
                 : "немає") // Тернарний оператор для виведення "є" або "немає"
  ...
  for (int i = 0; i < unique_count; i++) {
    cout << "Кількість місць: " << capacities[i] << " зустрічається "
         << capacity_count[i] << " раз(и)\n";
  }
}

/**
 * @brief Підраховує, скільки разів зустрічається кожна унікальна місткість
 * аудиторії.
 ...
 * @param unique_count Змінна, що зберігає кількість знайдених унікальних
 * місткостей (передається за посиланням).
 */
...
int jumpSearch(Audience *aud, int count, int target) {
  ...
  while (prev < count && aud[myMin(step, count) - 1].number < target) {
    ...
    step += sqrt(count);
    if (prev >= count)
      return -1; // Якщо вийшли за межі масиву, елемента немає
  }
