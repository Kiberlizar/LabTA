 ...
 */
void sortCapacitiesManual(int *capacities, int *capacity_count,
                          int unique_count) {
  int i = 0;
  while (i < unique_count) {
    // Рухаємось вперед, якщо це перший елемент або порядок правильний (за
    // спаданням)
...
int main() {
  ...
  // Для роботи Jump Search масив ОБОВ'ЯЗКОВО має бути відсортований за ключем
  // пошуку (номером)
  gnomeSort(aud, count, compareByNumberAsc);
